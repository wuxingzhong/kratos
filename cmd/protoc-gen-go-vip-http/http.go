...
func generateFile(gen *protogen.Plugin, file *protogen.File, omitempty bool, omitemptyPrefix string) *protogen.GeneratedFile {
	...
	g.P("// Code generated by protoc-gen-go-vip-http. DO NOT EDIT.")
	g.P("// versions:")
	g.P(fmt.Sprintf("// - protoc-gen-go-vip-http%s", release))
...
func replacePath(name string, value string, path string) string {
	...
	if len(idx) > 0 {
		// 修复: 使用 [^/]+ 而不是 .*
		regexValue := strings.ReplaceAll(value, "*", "[^/]+")
		if regexValue == "" {
			regexValue = "[^/]+" // 默认匹配非斜杠字符
		}
